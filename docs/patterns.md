...
Duplication has now slipped past a correctness check five times, and every time
...
| `promote` nesting check | `set` of promoted episode ids | is any promoted episode here twice? |
...
### When to check, which is the part the rule was missing

Both instances were written by someone who had read this entry. The second was
written the same day as it — `promote.py` used a set of `episode_id` to answer
"do these runs nest", which cannot see the same id twice, so a doubled row pooled
into a comparison and weighted one scenario double. Found by an audit, not by
knowing the rule.

So "match the container to the question" is not actionable on its own, because
it tells you what to get right and not when to look. Both instances share a
trigger, and it is specific enough to act on:

> **At write time, for any collection keyed by an identity.** The moment you
> write `{r["episode_id"] for r in rows}` or `dict` keyed by a hash, ask what the
> collection is being asked. "Which are present" is a set. "Is any present twice"
> is not, and identity keys are exactly where the second question is both likely
> and invisible.

The reason identity keys are the trigger: a set keyed by something incidental
loses information nobody wanted. A set keyed by an identity loses the fact that
the identity *repeated*, which is a defect somewhere upstream — a resumed run
writing a second part file, a scenario generated by two sets, a promoted run
pooled twice. The collection silently repairs the symptom and destroys the
evidence.

Checking at write time also costs nothing. `verify_written` compares multisets
because the question was asked while the function was being written; `promote.py`
did not, and the cost was an audit finding plus a second function.
